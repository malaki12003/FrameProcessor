...
std::string FrameProcessor::frameInfoListToJsonL(const std::vector<FrameInfo>& frameInfoList) const{
      std::string jsonLines;
    for (const FrameInfo& frameInfo : frameInfoList) {
        std::string jsonLine = "{\"frame\": " + std::to_string(frameInfo.frame) + ", \"rgb\": ["+std::to_string(frameInfo.rgb[0])+", "+std::to_string(frameInfo.rgb[1])+", "+std::to_string(frameInfo.rgb[2])+"]}";
        jsonLines += jsonLine + '\n';
    }
    return jsonLines;
}